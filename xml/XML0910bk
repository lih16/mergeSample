...
public static   class XmlRecordReader extends RecordReader<Text ,Text>{  
    ...
    public void initialize(InputSplit inputSplit, TaskAttemptContext cxt)  
        ...
        else {end = start + split.getLength(); 
dPath =path;		
		}
...
String symbol=line.toString().substring(0,2);
	    ...
	    if(symbol.equals("#CHROM")){ 		
			...
			value.set(samplename);
			return true;
        }  	
        else if(line.toString().indexOf("NON_REF")<0){
			key.set("NA");
			value.set("NA"); 
			return true;
		}		
        ...
        key.set(words[0]+"#"+words[1]+"#"+words[2]+"#"+words[3]+"#"+words[4]+"#"+words[5]+"#"+words[6]+"#"+words[7]+"#"+words[8]);  
        //value.set(words[6]+"|"+words[9]);  
        value.set(samplename+"#"+words[9]);  
        return true;  
...
private void codecWiseDecompress(Configuration conf) throws IOException{
		 ...
		 CompressionCodec codec = factory.getCodec(path);
		    ...
		    String outputUri = 
CompressionCodecFactory.removeSuffix(path.toString(), codec.getDefaultExtension());
